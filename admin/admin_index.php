...
if (!isset($_SESSION['username'])) {
    ...
    echo "<script>
            ...
            location.href='../index.php'; // Redirect ke halaman login
          </script>";
    exit();
}
...
<body>
  ...
  <div class="container mt-2">
    <div class="row">
      <?php
      // Menampilkan daftar foto yang di-upload
      $query = mysqli_query($koneksi, "SELECT * FROM foto 
                                        INNER JOIN user ON foto.id_user = user.id_user 
                    ...
                    <div class="col-md-4">
                      <div class="m-2">
                        <div class="overflow-auto">
                          ...
                          <?php
                          $id_foto = $data['id_foto'];
                          $komentar = mysqli_query($koneksi, "SELECT * FROM komentar_foto 
                                                                INNER JOIN user ON komentar_foto.id_user = user.id_user 
                                                                WHERE komentar_foto.id_foto='$id_foto'");
                          while ($row = mysqli_fetch_array($komentar)) {
                              // Cek apakah ini komentar milik user yang login atau tidak
                              $is_comment_owner = ($row['id_user'] == $id_user);
                          ?>
                              <p align="left">
                                  <strong><?php echo $row['nama'] ?></strong>: <?php echo $row['isi_komentar'] ?>
                                  <?php if ($_SESSION['role'] == 'admin' || $is_comment_owner) { // Admin atau pemilik komentar bisa menghapus ?>
